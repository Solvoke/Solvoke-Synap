...
我们深知您的代码库和 AI 对话包含高度敏感的知识产权与商业机密。

Synap 的架构严格遵循 **零遥测（Zero-telemetry）** 与 **本地优先（Local-first）** 原则：

- **直连传输：** 您的对话数据仅在浏览器/IDE 插件与您自托管的 API 服务器之间点对点传输。
- **无中间人：** 数据采集中不涉及任何中继服务器、分析追踪器或第三方 API。
- **开源可审计：** 核心数据处理层（`synap-web` 与 `@synap/core`）采用 AGPL-3.0 协议彻底开源，您可以逐行审计数据的处理逻辑。
...
欢迎贡献！请阅读 [CONTRIBUTING.md](packages/web/CONTRIBUTING.md) 了解开发环境搭建、分支工作流、CI/CD 详情和编码规范。

概览：

- 代码、注释和 commit message 使用**英文**
- Commit 格式：[Conventional Commits](https://www.conventionalcommits.org/)（如 `feat: add search filter`）
- 所有变更通过 Pull Request 进行，CI 检查通过后方可合并
