...
if (files.length === 0) {
  console.warn("[Warn] No preset CSS files found. Only default preset will be included.");
}
...
const presets = files.map((file) => {
  ...
  if (!labelMatch) {
    console.warn(`[Warn] No 'label:' found in ${file}, using filename as fallback.`);
  }
  if (!valueMatch) {
    console.warn(`[Warn] No 'value:' found in ${file}, using filename as fallback.`);
  }
  ...
  if (!lightPrimaryMatch || !darkPrimaryMatch) {
    console.warn(`[Warn] Missing --primary for ${file} (light or dark). Check CSS syntax.`);
  }
...
try {
  ...
  console.error(`[Error] Could not read globals.css at ${globalStylesPath}`);
  console.error(err);
  process.exit(1);
}
...
async function main() {
  ...
  if (formatted === fileContent) {
    console.log("[Info] No changes in theme.ts");
    return;
  }

  fs.writeFileSync(outputPath, formatted);
  console.log("[Done] theme.ts updated with new theme presets");
}

main().catch((err) => {
  console.error("[Error] Unexpected error while generating theme presets:", err);
  process.exit(1);
});
